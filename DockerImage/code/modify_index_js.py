...
def search_and_replace():
    ...
    for dirpath, dirnames, filenames in os.walk(filter_path):
        if "lambda" in dirnames:
            lambda_dir = os.path.join(dirpath, "lambda")
            for root, dirs, files in os.walk(lambda_dir):
                parts = root.split('/')
                print(parts)
                replace_path = chatGPT_path + "/" + parts[4] + "/" + parts[5] + "/" + "report.txt"
                print(replace_path)
                try:
                ...
                for file in files:
                    if file == "index.js":
                        # print(root, file)
                        try:
                            result = replace_code(os.path.join(root, file), os.path.join(root, "index.js"), replace_content)
                            if result:
                                found_count += 1
                            else:
                                not_found_count += 1
